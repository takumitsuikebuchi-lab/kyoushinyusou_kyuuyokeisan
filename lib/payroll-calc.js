 ...
 *
 * ⚠️ 【毎年要確認 — 通常4月施行】以下の定数は税制改正で変わります:
 *   - startThreshold      : 課税最低限額（月額）
 *   - depReductionPerPerson: 扶養1人あたりの月額控除
 *   - basicDedDefault     : 基礎控除（月額・標準）
 *   - basicDed 高所得段階値  : 基礎控除の段階的逓減額
 *   - TAX_TABLE_R7 / TAX_TABLE_R8: 税額表そのもの
 *   新年度対応時は国税庁「給与所得の源泉徴収税額表」で必ず確認してください。
 *   新しい年度を追加する場合は TAX_TABLE_RXX を定義し、下記の useR7 分岐を拡張してください。
 */
export const estimateTax = (taxable, deps, taxYear) => {
  if (taxable <= 0) return 0;

  // ── 税年による税額表・パラメータの切替 ──────────────────────────────────
  // ⚠️ 新年度対応時: TAX_TABLE_RXX 追加 + 下記定数に新しい三項演算子分岐を追加
  const useR7 = taxYear === 2025; // 令和7年（2025年1月〜12月支給分）
  // ※ taxYear が 2025 以外はすべて令和8年（R8）として扱う。2027年分が必要になったら要拡張。
  const table = useR7 ? TAX_TABLE_R7 : TAX_TABLE_R8;
  const startThreshold = useR7 ? 88000 : 105000;          // R7: 88,000円 / R8: 105,000円
  const depReductionPerPerson = useR7 ? 1580 : 1610;      // R7: 1,580円  / R8: 1,610円（扶養1人あたり）
  const basicDedDefault = useR7 ? 40000 : 48334;          // R7: 40,000円 / R8: 48,334円（基礎控除・標準）
  ...
  if (taxable < startThreshold) {
    ...
    // 740,000円以上: 電算機計算の特例（国税庁「電算機計算の特例」に基づく）
    // ⚠️ incDed の算式・閾値（158333, 299999, 549999, 708330）はR7・R8で共通。
    //    税制改正で変更される場合は国税庁の最新告示を確認してください。
    let incDed;
    if (taxable <= 158333) incDed = 54167;                        // 〜1,900,000円/年
    else if (taxable <= 299999) incDed = taxable * 0.30 + 6667;  // 〜3,600,000円/年
    else if (taxable <= 549999) incDed = taxable * 0.20 + 36667; // 〜6,600,000円/年
    else if (taxable <= 708330) incDed = taxable * 0.10 + 91667; // 〜8,500,000円/年
    else incDed = 162500;                                          // 8,500,000円/年超
    incDed = Math.ceil(incDed);
    // ⚠️ basicDed の段階逓減値（40000 / 26667 / 13334）はR7基準。
    //    R8以降で基礎控除額が変わった場合はこれらの値も更新が必要です。
    //    (段階逓減の適用所得：月額2,120,834円〜2,245,833円 ≒ 年収2,550万〜2,695万円)
    const basicDed = taxable <= 2120833 ? basicDedDefault
      : taxable <= 2162499 ? 40000   // ⚠️ 要年度確認
        : taxable <= 2204166 ? 26667 // ⚠️ 要年度確認
          : taxable <= 2245833 ? 13334 : 0; // ⚠️ 要年度確認
    const depDed = deps * 31667; // 扶養控除 380,000円/年 → 31,667円/月（R7・R8共通）
...
export const calcPayroll = (emp, att, settings, options = {}) => {
  ...
  // avgMonthlyHours が未設定・ゼロの場合は時間単価 0 で計算（÷0 / NaN / Infinity 防止）
  const hourly = emp.avgMonthlyHours > 0
    ? (emp.basicPay + emp.dutyAllowance) / emp.avgMonthlyHours
    : 0;
  // 固定残業（みなし残業）対応
  const hasFixedOT = (emp.fixedOvertimeHours || 0) > 0 && (emp.fixedOvertimePay || 0) > 0;
  let otLegal, otPrescribed, otNight, otHoliday, fixedOvertimePay, excessOvertimePay;
  if (hasFixedOT) {
    // 固定残業制: 固定残業代は定額支給、超過分のみ追加計算
    fixedOvertimePay = emp.fixedOvertimePay;
    const totalActualOT = (att.legalOT || 0) + (att.prescribedOT || 0);
    const excessHours = Math.max(0, totalActualOT - emp.fixedOvertimeHours);
    // 超過分は法定外残業の割増率(1.25)で計算
    excessOvertimePay = excessHours > 0 ? ceil(hourly * excessHours * 1.25) : 0;
    // 個別残業手当はゼロ（固定残業代 + 超過分に統合）
    otLegal = 0;
    otPrescribed = 0;
    // 深夜・休日は固定残業の対象外（別途計算）
    otNight = calcOvertime(hourly, att.nightOT, 1.25);
    otHoliday = calcOvertime(hourly, att.holidayOT, 1.35);
  } else {
    fixedOvertimePay = 0;
    excessOvertimePay = 0;
    otLegal = calcOvertime(hourly, att.legalOT, 1.25);
    otPrescribed = calcOvertime(hourly, att.prescribedOT, 1.0);
    otNight = calcOvertime(hourly, att.nightOT, 1.25);
    otHoliday = calcOvertime(hourly, att.holidayOT, 1.35);
  }
  ...
  const gross = (emp.basicPay + basicPayAdj) + emp.dutyAllowance + fixedOvertimePay + excessOvertimePay + otLegal + otPrescribed + otNight + otHoliday + otAdjust + otherAllowance + emp.commuteAllow;
  // stdMonthly が未設定・ゼロ・負の場合は 0 扱い（社保計算の NaN / 負値を防止）
  const stdMonthly = emp.stdMonthly > 0 ? emp.stdMonthly : 0;
  const health = stdMonthly ? insRound(stdMonthly * rates.health) : 0;
  const kaigo = emp.hasKaigo && stdMonthly ? insRound(stdMonthly * rates.kaigo) : 0;
  const pension = emp.hasPension && stdMonthly ? insRound(stdMonthly * rates.pension) : 0;
  ...
  const incomeTax = (emp.incomeTaxOverride != null && emp.incomeTaxOverride !== "")
    ? Number(emp.incomeTaxOverride)
    : estimateTax(taxableIncome, emp.dependents, options.taxYear);
  const residentTax = emp.residentTax;
  const totalDeduct = socialTotal + incomeTax + residentTax;
  const netPay = gross - totalDeduct;
  // 事業主負担
  const erHealth = stdMonthly ? insRound(stdMonthly * rates.healthEr) : 0;
  const erKaigo = emp.hasKaigo && stdMonthly ? insRound(stdMonthly * rates.kaigoEr) : 0;
  const erPension = emp.hasPension && stdMonthly ? insRound(stdMonthly * rates.pensionEr) : 0;
  const erChildCare = stdMonthly ? insRound(stdMonthly * rates.childCare) : 0;
  ...
  return { hourly: Math.round(hourly * 100) / 100, otLegal, otPrescribed, otNight, otHoliday, otAdjust, basicPayAdj, otherAllowance, fixedOvertimePay, excessOvertimePay, hasFixedOT, gross, health, kaigo, pension, employment, socialTotal, incomeTax, residentTax, totalDeduct, netPay, erHealth, erKaigo, erPension, erChildCare, erEmployment, erTotal, companyCost };
};
...
export const calcBonusTax = (bonusAmount, prevMonthSocialDeducted, deps, settings, emp) => {
  ...
  // 扶養人数に応じて「前月社保控除後給与」をシフトしてから税率テーブルを引く
  // 国税庁「賞与に対する源泉徴収税額の算出率の表」の正式な計算方法:
  //   扶養0人=0, 1人=8,333円, 2人=16,667円, 3人以上=8,334円/人×人数-1
  //   を前月社保後給与から差し引いてからテーブルを参照する
  const depAdjustment = deps <= 0 ? 0
    : deps === 1 ? 8333
      : deps === 2 ? 16667
        : 8334 * deps - 1;
  const adjustedPrevMonthPay = Math.max(0, prevMonthSocialDeducted - depAdjustment);

  let rate = 0;
  for (const [threshold, r] of BONUS_TAX_RATE_TABLE) {
    if (adjustedPrevMonthPay < threshold) { rate = r; break; }
  }
  const bonusTax = Math.round(taxable * rate / 10) * 10;

  return { bonusTax, bHealth, bKaigo, bPension, bEmployment, bSocialTotal, taxable, rate, adjustedPrevMonthPay };
};
