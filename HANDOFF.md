...
> **最終更新**: 2026-02-16
> **最終更新者**: GenSpark → Claude Code へ引継ぎ
> **最終コミット**: `feat: 大規模リファクタリング — lib統合・R7税表・賞与計算・年末調整・監査ログUI・バックアップUI・Next.js 15.5.12`
> **ステータス**: lib/payroll-calc.js にロジック統合完了。Next.js 15.5.12へアップグレード（脆弱性0件）。次は (C) taxYear実接続 → (D) 賞与UI → (E) 年末調整UI → (F) page.jsx分割。
...
- 現在ブランチ: `main`
- 最新コミット: `feat: 大規模リファクタリング — lib統合・R7税表・賞与計算・年末調整・監査ログUI・バックアップUI・Next.js 15.5.12`
- 次に着手する課題: (C) calcPayroll呼出にtaxYear渡し → (D) 賞与計算UI → (E) 年末調整UI → (F) page.jsx分割
- プレビューURL: `http://127.0.0.1:3000`（ローカル） → Vercelデプロイ後 `https://xxxxx.vercel.app`
- 検証結果: `npm run build` 成功（Next.js 15.5.12）。`npm audit` 脆弱性0件。Middleware 82 kB。
...
| 優先度 | ID | 項目 | 詳細 |
|--------|-----|------|------|
| **高** | C | **taxYearFromPayMonth の実接続** | `calcPayroll()` 呼出箇所に `options.taxYear` を渡していない。2025年支給分にR7表が適用されない。page.jsx 内の全 `calcPayroll()` 呼出に `{ taxYear: taxYearFromPayMonth(payrollTargetMonth) }` を追加する |
...
- **105,000円未満**: 税額0
- **105,000〜740,000円未満**: テーブルルックアップ
- **740,000円以上**: 電算機計算の特例（基礎控除48,334円/月）
...
31. **taxYearFromPayMonth が未接続** — `calcPayroll()` の呼出箇所で `options.taxYear` を渡していないため、現状すべてR8テーブルが使用される。2025年支給分の正確な計算には課題Cの対応が必要。
