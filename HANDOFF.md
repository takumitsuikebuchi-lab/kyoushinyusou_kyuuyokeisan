...
> **最終更新**: 2026-02-13
> **最終更新者**: Antigravity (Gemini)
> **最終コミット**: `feat: 伊達良幸の従業員仮登録 + run-monthly API廃止`
> **ステータス**: 伊達良幸を仮登録済み（基本給・保険未設定）。run-monthly APIを廃止スタブに置換。次は伊達良幸の正式データ設定とMF元CSV突合。
...
- 現在ブランチ: `main`
- 最新コミット: `feat: 伊達良幸の従業員仮登録 + run-monthly API廃止`
- 次に着手する課題: `伊達良幸の正式データ設定`（基本給・保険等）→ `MF元CSV実データで最終一致確認`
- プレビューURL: `http://127.0.0.1:3000`
- 検証結果: `npm run dev` で動作確認済み（ダッシュボード・従業員一覧・給与明細一覧で伊達良幸の表示を確認）
...
| 優先度 | 項目 | 詳細 |
|--------|------|------|
| **高** | 伊達良幸の正式データ設定 | 仮登録済み（ID: `hrmos_22`）。基本給・職務手当・標準報酬月額・社会保険加入状況を正式値に更新する |
| **中** | MF元CSV実データで最終一致確認 | 突合レポート機能は実装済み。2026-01の実CSVを取込んで総支給・控除・差引が0差分であることを確認する |
