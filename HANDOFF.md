...
> **最終更新**: 2026-02-11
> **最終更新者**: Codex
> **最終コミット**: `feat: 履歴画面から対象月の再計算・スナップショット更新導線を追加`
> **ステータス**: 履歴画面で現在対象月の「再計算して更新」が実行可能。次は実データの `2026-01` 再同期/確定でスナップショットを更新。
...
- 現在ブランチ: `main`
- 最新コミット: `feat: 履歴画面から対象月の再計算・スナップショット更新導線を追加`
- 次に着手する課題: `2026-01スナップショット実データ更新`（HRMOS同期または手入力後に「この月を再計算して更新」を実行）
...
| 優先度 | 項目 | 詳細 |
|--------|------|------|
| **高** | 2026-01スナップショット実データ更新 | 導線は追加済み。実データの更新は未実施のため、HRMOS同期+「この月を再計算して更新」または確定を実行する |
